	...
	static async create(app){
		...
		if (this.__dependencies){
			...
			for (let i = 0; i < this.__dependencies.length; i++) {
				switch (this.__dependencies[i]){
					case 'resource':
						routerParams.push(resource.bind(resources));
					break;
					case 'route':
						routerParams.push(routeBinded);
					break;
					case 'rawRouter':
						routerParams.push(app.httpApp);
					break;
					default:
				}
			}
		} else {
			routerParams = [resource.bind(resources), routeBinded];
		}
		router = new this(...routerParams);
			...
			if (router.controllers[route.controller] && typeof router.controllers[route.controller].default === 'function'){
				...
				if (view && typeof ControllerForMethod.prototype[method] === 'function' && typeof view.render === 'function'){
					...
					while (parent !== Controller.Base){
						if (parent['__beforeHandlers__' + method]){
							beforeHandlers = beforeHandlers.concat(parent['__beforeHandlers__' + method]);
						}
						if (parent.__beforeHandlers){
							beforeHandlers = beforeHandlers.concat(parent.__beforeHandlers);
						}
						if (parent['__afterHandlers__' + method]){
							afterHandlers = afterHandlers.concat(parent['__afterHandlers__' + method]);
						}
						if (parent.__afterHandlers){
							afterHandlers = afterHandlers.concat(parent.__afterHandlers);
						}
						parent = Object.getPrototypeOf(parent);
					}
					beforeHandlers = beforeHandlers.filter(item => (typeof item !== 'string' || ControllerForMethod.prototype[item]))
						.map(item => ((typeof item === 'string') ? ControllerForMethod.prototype[item] : item));

					afterHandlers = afterHandlers.filter(item => (typeof item !== 'string' || ControllerForMethod.prototype[item]))
						.map(item => ((typeof item === 'string') ? ControllerForMethod.prototype[item] : item));
					...
					if ((beforeHandlers.length === 0) && (afterHandlers.length === 0 )){
						...
						handler = async (req, res) => {
							let controller = new ControllerForMethod();
							console.log(beforeHandlers)
							for (let i = 0; i < beforeHandlers.length; i++) {
								await beforeHandlers[i].call(controller, req.params);
							}
							await controller[method](req.params);
							for (let i = 0; i < afterHandlers.length; i++) {
								await afterHandlers[i].call(controller, req.params);
							}
							res.status(200).send(view.render(controller));
						};
					}

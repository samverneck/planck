...
let getBeforeHandlers = function(controller, method){
	let getBeforeHandlersRecursion = function(parent){
		...
		if (Object.getOwnPropertySymbols(parent).indexOf(symbols.beforeHandlers) !== -1){
			beforeHandlers = parent[symbols.beforeHandlers].concat(beforeHandlers);
		}
		if (Object.getOwnPropertySymbols(parent).indexOf(symbols.skipBeforeHandlers) !== -1){
...
let getAfterHandlers = function(controller, method){
	let getAfterHandlersRecursion = function(parent){
		...
		if (Object.getOwnPropertySymbols(parent).indexOf(symbols.afterHandlers) !== -1){
			afterHandlers = parent[symbols.afterHandlers].concat(afterHandlers);
		}
		if (Object.getOwnPropertySymbols(parent).indexOf(symbols.skipAfterHandlers) !== -1){
	...
	static async create(app){
		...
		if (app.config.codeGeneration.autoGeneration){
			...
			for (let [name, controller] of resources.controllers.entries()){
				dataToGenerate.push(CodeGenerator.generateIfNotExists('controller', {name: name, ...controller}, app.config.controllers.path, app.config.controllers.template));
			}
		...
		for (let i = 0; i < moduleNames.length; i++) {
			try{
				router.controllers[moduleNames[i]] = await System.import(pathModule.join(process.cwd(), app.config.controllers.path, moduleNames[i]));
			} catch(err) {
				console.error(err);
			}
		}
					...
					let handler = async (req, res) => {
						...
						try{
							...
							for (let i = 0; i < beforeHandlers.length; i++) {
								await Reflection.invoke(beforeHandlers[i], controller, {req: req, res: res, request: req, response: res, params: req.params});
							}
							await Reflection.invoke(controller[method], controller, {req: req, res: res, request: req, response: res, params: req.params});
							for (let i = 0; i < afterHandlers.length; i++) {
								await Reflection.invoke(afterHandlers[i], controller, {req: req, res: res, request: req, response: res, params: req.params});
							}

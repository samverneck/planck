...
class Generator{
	...
	async run(cliParams){
		...
		console.log(`${'creating'.padStart(10)}  ${cliParams[0]}`);
		...
		let render =  function(dir){
			(fs.readdirSync(path.join(__dirname, 'templates', dir))).map((templatePath) => {
				if (( fs.statSync(path.join(__dirname, 'templates', dir, templatePath))).isDirectory()){
					try {
						console.log(`${'creating'.padStart(10)}  ${templatePath.padStart(templatePath.length+(dir.length?2:0)+dir.split(path.sep).length*2)}`);
					...
					try {
						...
						console.log(`${'creating'.padStart(10)}  ${newName.padStart(newName.length+(dir.length?2:0)+dir.split(path.sep).length*2)}`);

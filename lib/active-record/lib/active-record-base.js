...
class Base{
	...
	static async init(){
		...
		for (let i in instance){
				...
				(typeof this['getBy' + i.charAt(0).toUpperCase() + i.slice(1)] !== 'function'))
			{
				this['getBy' + i.charAt(0).toUpperCase() + i.slice(1)] = function(...params){
					return chainableMethod.call(this, 'getBy', params, 'getBy');
				};
			}
		}
		...
		this.fields = function(...params) {
			switch (this.state || 'initial'){
				case 'getAll':
				case 'getMultipleByID':
				case 'getSingleByID':
					if (params.length > 1) {
						return chainableMethod.call(this, 'fields', params);
					} else if (typeof params[0] !== 'string'){
						return chainableMethod.call(this, 'fields', params[0]);
					} else {
						return chainableMethod.call(this, 'fields', params);
					}
				break;
				default :
					throw new Error(`Using 'fields' method is not allowed`);
			}
		};

	...
	calcQuery(chain, state){
		...
		for (let block of chain) {
			switch (block.method){
				...
				break;
				case 'fields': 
					if (block.params instanceof Array) {
						query.options.fields = {};
						block.params.forEach((field) => {
							query.options.fields[field] = 1;
						});
					} else if (typeof block.params === 'object') {
						query.options.fields = block.params;
					} else {
						throw new TypeError('Arguments to Base.fields must be array of strings or Object');
					}					
				break;
				default:
			}

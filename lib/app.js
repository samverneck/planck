...
const patchLoader = function(){
	if (this.config.codeGeneration.autoGeneration){
		const oldLocate = System.locate;
		const app = this;
		System.locate = async function(params){
			let configPath = pathModule.join(process.cwd(), app.config.models.path);
			let path = await System.normalize(configPath);
			path = path.substr(0, path.length - '.js'.length);
			let index = params.name.indexOf(path);
			if (index !== -1){
				let name = params.name.slice(index + path.length, params.name.length);
